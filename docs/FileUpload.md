...
Berikut adalah contoh praktis dari awal hingga akhir (*end-to-end*) cara membuat form komponen UI untuk mengunggah gambar dan menyimpannya di Convex.
  ...
  const handleUploadDanSimpan = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!presignedData.success) {
        throw new Error(presignedData.error || "Gagal mendapatkan presigned URL");
      }
...
const hapusDataBerita = async (idBerita: string, fileKeyGambar: string) => {
  try {
    ...
    if (fileKeyGambar) {
      const deleteResult = await deleteFileFromR2({ data: { fileKey: fileKeyGambar } });
...
1. **Efisiensi Cloudflare R2**: Karena R2 tidak memungut biaya egress (gratis lalu lintas keluar data), teknik *direct client upload* ini meminimalkan biaya *workers invocation* dari Cloudflare Pages.
