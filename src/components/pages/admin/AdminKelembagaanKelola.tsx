...
import { Save, ArrowLeft, Loader2, Landmark, Plus, Trash2, Pencil as Edit, X } from "lucide-react";
...
export default function AdminKelembagaanKelola({ lembagaId, initialData }: AdminKelembagaanKelolaProps) {
  ...
  const pengurusList = useQuery(api.kelembagaan.getPengurusByKelembagaan, { kelembagaanId: lembagaId });
  ...
  const [originalLogoKey, setOriginalLogoKey] = useState<string | undefined>(initialData.logoKey);
  ...
  const [editPengurusId, setEditPengurusId] = useState<Id<"pengurus_kelembagaan"> | null>(null);
  ...
  const handleSaveLembaga = async (e: React.FormEvent) => {
    ...
    try {
      if (originalLogoKey && originalLogoKey !== lembagaForm.logoKey) {
        try { await deleteFileFromR2({ data: { fileKey: originalLogoKey } }); } 
        catch (err) { console.error("Gagal hapus logo lama", err); }
      }
          ...
          <Card className="shadow-sm border-slate-200 sticky top-4">
            ...
            <CardContent className="pt-6">
              <form onSubmit={handleSaveLembaga} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <div className="p-2 border rounded-lg bg-slate-50">
                    <ImageUpload 
                      ...
                      onChange={(url, key) => setLembagaForm({...lembagaForm, logoUrl: url, logoKey: key})} 
                ...
                <div className="space-y-2">
                  <Label htmlFor="nama">Nama Lembaga <span className="text-red-500">*</span></Label>
                  <Input 
                    id="nama" 
                    value={lembagaForm.nama} 
                    onChange={e => setLembagaForm({...lembagaForm, nama: e.target.value})} 
                    required 
                  />
                </div>
                
                <div className="space-y-2">
                  <Label htmlFor="singkatan">Singkatan <span className="text-red-500">*</span></Label>
                  <Input 
                    id="singkatan" 
                    value={lembagaForm.singkatan} 
                    onChange={e => setLembagaForm({...lembagaForm, singkatan: e.target.value})} 
                    required 
                  />
                </div>
                ...
                <Button type="submit" disabled={isSubmittingLembaga} className="w-full gap-2">
                  {isSubmittingLembaga ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
                  Simpan Profil
                </Button>
              </form>
            </CardContent>
          ...
          <Card className="shadow-sm border-slate-200">
            <CardHeader className="bg-slate-50/50 border-b flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                ...
                <CardDescription>Tambah, edit, dan kelola urutan anggota.</CardDescription>
              </div>
            </CardHeader>
            <CardContent className="p-0">
              {/* Form Input Inline */}
              <div className="p-4 bg-blue-50/50 border-b">
                <form onSubmit={handleSavePengurus} className="flex flex-col sm:flex-row items-end gap-3">
                  ...
                  <div className="space-y-1.5 w-full sm:w-24">
                    <Label className="text-xs font-semibold text-slate-700">No. Urut</Label>
                    <Input 
                      type="number" 
                      value={pengurusForm.urutan} 
                      onChange={e => setPengurusForm({...pengurusForm, urutan: parseInt(e.target.value) || 1})} 
                      min={1} 
                      required 
                  ...
                  <div className="flex gap-2 w-full sm:w-auto">
                    {editPengurusId && (
                      <Button type="button" variant="ghost" onClick={cancelEditPengurus} className="w-full sm:w-auto h-10 text-slate-500 hover:text-slate-700 hover:bg-slate-200/50">
                        <X className="w-4 h-4" />
                      </Button>
                    )}
                    <Button type="submit" disabled={isSubmittingPengurus} className={`w-full sm:w-auto h-10 gap-2 ${editPengurusId ? 'bg-amber-500 hover:bg-amber-600 text-white' : 'bg-blue-600 hover:bg-blue-700'}`}>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-slate-50/50">
                    <TableHead className="w-16 text-center font-semibold">Urut</TableHead>
                ...
                <TableBody>
                  {pengurusList === undefined ? (
                    ...
                    <TableRow>
                      <TableCell colSpan={4} className="h-32 text-center text-slate-500 bg-slate-50/30">
                        Belum ada anggota pengurus. Silakan tambahkan melalui form di atas.
                      </TableCell>
                    </TableRow>
                  ) : (
                    pengurusList.map(p => (
                      <TableRow key={p._id} className={editPengurusId === p._id ? "bg-blue-50/30" : ""}>
                        <TableCell className="text-center font-medium text-slate-500">{p.urutan}</TableCell>
                        <TableCell className="font-medium text-slate-800">{p.nama}</TableCell>
                        <TableCell className="text-slate-600">{p.jabatan}</TableCell>
