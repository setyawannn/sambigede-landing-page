...
export default function AdminBumdes() {
  const bumdesList = useQuery(api.bumdes.getBumdesList)
  const deleteBumdes = useMutation(api.bumdes.deleteBumdes)

  // Delete State
  const [deleteItem, setDeleteItem] = useState<Doc<'bumdes'> | null>(null)
  ...
  const totalItems = bumdesList?.length || 0
  const totalPages = Math.ceil(totalItems / itemsPerPage)
  const paginatedBumdes =
    bumdesList?.slice(
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (deleteItem.fotoProdukKeys) {
        for (const key of deleteItem.fotoProdukKeys) {
          await deleteFileFromR2({ data: { fileKey: key } })
        }
      }
      await deleteBumdes({ id: deleteItem._id })
      toast.success('BUMDes berhasil dihapus')
      setDeleteItem(null)
    } catch (error: any) {
      toast.error(error.message || 'Gagal menghapus BUMDes')
    } finally {
      setIsDeleting(false)
    }
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            Kelola data dan informasi unit usaha BUMDes Sambigede.
          </p>
        </div>
        <Link to="/admin/konten/bumdes/tambah">
          <Button className="gap-2">
            <Plus className="w-4 h-4" /> Tambah BUMDes
          </Button>
        </Link>
      </div>
      ...
      <Card>
        <CardContent className="p-0 overflow-x-auto">
          <Table>
            <TableHeader className="bg-slate-50">
              <TableRow>
                ...
                <TableHead>Status Hukum</TableHead>
                <TableHead className="text-right">Aksi</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {bumdesList === undefined ? (
                <TableRow>
                ...
                <TableRow>
                  ...
                  >
                    Belum ada data BUMDes yang ditambahkan.
                  </TableCell>
                </TableRow>
              ) : (
                paginatedBumdes.map((item, index) => (
                  <TableRow key={item._id}>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <Link
                          to="/admin/konten/bumdes/kelola/$id"
                          params={{ id: item._id }}
                        >
                          <Button
                            variant="ghost"
          ...
          {/* Pagination Controls */}
          {bumdesList !== undefined && bumdesList.length > 0 && (
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Hapus Data BUMDes?</AlertDialogTitle>
            <AlertDialogDescription>
              Tindakan ini tidak dapat dibatalkan. Data BUMDes "
