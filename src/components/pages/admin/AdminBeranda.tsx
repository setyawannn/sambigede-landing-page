...
import { Save, Home, Image as ImageIcon, MessageSquare, Loader2, User } from "lucide-react";
...
export default function AdminBeranda() {
  ...
  const perangkatData = useQuery(api.perangkat.getPerangkatList, { status: "Aktif" });

  const [heroBadge, setHeroBadge] = useState("");
  const [heroTitle, setHeroTitle] = useState("");
  const [heroSubtitle, setHeroSubtitle] = useState("");
  const [heroImageUrl, setHeroImageUrl] = useState("");
  const [heroImageKey, setHeroImageKey] = useState<string | undefined>(undefined);
  const [originalHeroImageKey, setOriginalHeroImageKey] = useState<string | undefined>(undefined);
    ...
    <div className="space-y-6 pb-10">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Kelola konten Hero Banner dan Sambutan Kepala Desa.</p>
        </div>
          ...
          <Card className="overflow-hidden">
            ...
            <CardContent className="pt-6 space-y-4">
              ...
              <div className="space-y-2">
                ...
                />
                <p className="text-xs text-slate-500">Gunakan baris baru (Enter) untuk memotong teks ke bawah.</p>
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2 pt-2">
                ...
                <p className="text-xs text-slate-500 mb-2">Rekomendasi rasio: 16:9 (Landscape) dengan kualitas tinggi.</p>
                <ImageUpload
                  value={heroImageUrl}
                  onChange={(url, key) => {
                    setHeroImageUrl(url);
            ...
            <CardContent className="pt-6 space-y-4">
              {kades ? (
                <div className="flex items-center gap-4 p-4 border border-[#E5E5E5] rounded-xl bg-slate-50 mb-4">
                  {kades.imageUrl ? (
                    <img src={kades.imageUrl} alt={kades.nama} className="w-16 h-16 rounded-full object-cover border border-slate-200 shadow-sm" />
                  ) : (
                  ...
                  <div>
                    <p className="text-sm text-slate-500 font-medium">Data Kepala Desa Aktif:</p>
                    <p className="text-lg font-bold text-slate-800">{kades.nama}</p>
                    <p className="text-xs text-slate-500">Diambil dari menu Perangkat Desa</p>
                  </div>
                </div>
              ) : (
                <div className="p-4 border border-amber-200 bg-amber-50 rounded-xl mb-4 text-amber-700 text-sm">
                  <strong>Peringatan:</strong> Tidak ditemukan Perangkat Desa yang berstatus "Aktif" dengan jabatan "Kepala Desa". Silakan tambahkan Kepala Desa terlebih dahulu di menu Perangkat Desa.
                </div>
              )}
