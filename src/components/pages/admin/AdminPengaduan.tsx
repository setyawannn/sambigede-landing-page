...
import { ShieldAlert, Trash2, MessageSquare, History, User, Clock, AlertTriangle, ListFilter, CalendarIcon, Download, X, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function AdminPengaduan() {
  ...
  const handleExportExcel = () => {
    ...
    const exportData = filteredPengaduan.map((item, index) => ({
      "No.": index + 1,
      "Tanggal Laporan": format(new Date(item._creationTime), "dd MMM yyyy, HH:mm", { locale: id }),
  ...
  const logStartItem = totalLogItems === 0 ? 0 : (logCurrentPage - 1) * logItemsPerPage + 1;
      ...
      <Tabs defaultValue="laporan" className="w-full">
        <TabsList className="grid w-full md:w-[400px] grid-cols-2">
          <TabsTrigger value="laporan">
            <ListFilter className="w-4 h-4 mr-2" /> Daftar Laporan
          </TabsTrigger>
          <TabsTrigger value="audit">
            <History className="w-4 h-4 mr-2" /> Log Aktivitas
          </TabsTrigger>
        </TabsList>
          ...
          <Card>
            <CardHeader className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              <div>
                ...
                <CardDescription>Lihat dan kelola laporan satu arah dari masyarakat</CardDescription>
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
                ...
                <Button 
                  ...
                  disabled={!filteredPengaduan || filteredPengaduan.length === 0}
                >
            ...
            <CardContent>
              {pengaduanList === undefined ? (
                ...
                <div className="text-center py-12 text-slate-500">
                  ...
                  <p>Belum ada laporan pengaduan masyarakat untuk filter tersebut.</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {currentPengaduan.map((p) => (
                    <div key={p._id} className="border border-slate-200 rounded-xl p-5 bg-white shadow-sm hover:shadow-md transition-shadow">
                      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                        <div className="space-y-3 flex-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <span className="text-xs text-slate-400 flex items-center gap-1">
                              ...
                              {format(new Date(p._creationTime), "dd MMM yyyy, HH:mm", { locale: id })}
                            </span>
                          </div>
                          
                          <div>
                            <h3 className="font-semibold text-lg text-slate-900">{p.namaLengkap}</h3>
                            <p className="text-sm text-slate-500">{p.emailOrPhone}</p>
                          </div>
                        ...
                        <div className="flex flex-row md:flex-col gap-2 shrink-0 border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-4">
                          <Button variant="outline" size="sm" onClick={() => setDeleteId(p._id)} className="text-red-600 hover:text-red-700 hover:bg-red-50 flex-1 md:flex-none">
        ...
        <TabsContent value="audit" className="mt-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Rekam jejak perubahan data pada tabel pengaduan dan master kategori</CardDescription>
            </CardHeader>
                ...
                <>
                          ...
                          <tr key={log._id} className="border-b hover:bg-slate-50">
                            ...
                            <td className="px-6 py-4">
                              <div className="font-medium text-slate-900 flex items-center gap-2">
                                <User className="w-4 h-4 text-slate-400" /> {log.adminNama}
                              </div>
                              <div className="text-xs text-slate-400">@{log.adminUsername}</div>
                            </td>
                            <td className="px-6 py-4">
                            ...
                            <td className="px-6 py-4 font-mono text-xs">{log.tableName}</td>
                            <td className="px-6 py-4">
                              <div className="max-w-sm max-h-24 overflow-y-auto text-xs bg-slate-100 p-2 rounded-md border">
                                {log.action === "UPDATE" && log.oldValue && log.newValue && (
                                  <div className="space-y-1">
                                    <div className="text-red-600 line-through">Lama: {log.oldValue}</div>
                                    <div className="text-green-600">Baru: {log.newValue}</div>
                                  </div>
                                )}
                                {log.action === "DELETE" && log.oldValue && (
                                  <div className="text-red-600">Data dihapus: {log.oldValue}</div>
                                )}
                                {log.action === "INSERT" && log.newValue && (
                                  <div className="text-green-600">Data baru: {log.newValue}</div>
                                )}
                              </div>
                            </td>
                          </tr>
                  ...
                  {totalLogItems > 0 && (
                    <div className="p-4 border-t border-slate-100 bg-slate-50/30 flex flex-col sm:flex-row items-center justify-between gap-4">
                      <div className="text-sm text-slate-500">
                        Menampilkan <span className="font-medium text-slate-700">{logStartItem}</span> sampai <span className="font-medium text-slate-700">{logEndItem}</span> dari <span className="font-medium text-slate-700">{totalLogItems}</span> data
                      </div>

                      <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6">
                        <div className="flex items-center gap-2 text-sm text-slate-500">
                          <span>Baris per halaman:</span>
                          <Select value={String(logItemsPerPage)} onValueChange={(val) => {
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              Aksi ini akan dicatat ke dalam log sistem dan tidak dapat dikembalikan.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
