...
export default function AdminBeritaForm({ mode, initialData }: AdminBeritaFormProps) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (mode === "edit" && initialData) {
      ...
      setFormData(prev => ({
        ...
        category: kategoriList.length > 0 ? kategoriList[0].nama : "Berita Desa"
      }));
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "edit" && initialData) {
        ...
        await updateBerita({ ...formData, id: initialData._id, author: initialData.author });
    ...
    <form onSubmit={handleSubmit} className="space-y-6 pb-10">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          <Button type="button" variant="outline" onClick={() => navigate({ to: "/admin/berita" })} disabled={isSubmitting}>
            Batal
          </Button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="overflow-hidden">
            <CardHeader className="bg-slate-50 border-b pb-4">
              <CardTitle className="text-lg text-slate-700">Konten Artikel</CardTitle>
            </CardHeader>
            <CardContent className="pt-6 space-y-6">
              <div className="space-y-2">
                <Label htmlFor="title" className="text-sm font-semibold">Judul Artikel <span className="text-red-500">*</span></Label>
                <Input 
                  id="title" 
              ...
              <div className="space-y-2">
                <Label htmlFor="excerpt" className="text-sm font-semibold">Kutipan Pendek (Excerpt) <span className="text-red-500">*</span></Label>
                <Textarea 
                  id="excerpt" 
                  ...
                  rows={2}
                />
                <p className="text-xs text-slate-500">Teks ini akan muncul di kartu cuplikan berita pada halaman depan.</p>
              </div>

              <div className="space-y-2">
                <Label htmlFor="content" className="text-sm font-semibold">Isi Konten Lengkap <span className="text-red-500">*</span></Label>
                <Textarea 
                  id="content" 
        ...
        <div className="space-y-6">
          <Card className="overflow-hidden">
            ...
            <CardContent className="pt-6">
              <div className="space-y-2">
                ...
                />
                <p className="text-xs text-slate-500 mt-2 text-center">Rekomendasi rasio: 16:9 (Landscape).</p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="overflow-hidden">
            <CardHeader className="bg-slate-50 border-b pb-4">
              <CardTitle className="text-lg text-slate-700">Meta Data</CardTitle>
            </CardHeader>
              ...
              <div className="space-y-2">
                ...
                <Select value={formData.category} onValueChange={val => setFormData({...formData, category: val})}>
                  ...
                  <SelectContent>
                    ...
                    {kategoriList.length === 0 && <SelectItem value="Uncategorized">Belum ada kategori</SelectItem>}
                  </SelectContent>
                </Select>
              </div>
