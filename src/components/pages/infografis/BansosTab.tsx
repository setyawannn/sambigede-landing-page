...
export default function BansosTab() {
  ...
  // Hitung KPI
  const kpi = useMemo(() => {
    if (!bansosList) return { total: 0, pkh: 0, bpnt: 0, lainnya: 0 }

    return bansosList.reduce(
      (acc, curr) => {
        acc.total++
        if (curr.jenisBansos === 'PKH') acc.pkh++
        else if (curr.jenisBansos === 'BPNT') acc.bpnt++
        else acc.lainnya++
        return acc
      },
      { total: 0, pkh: 0, bpnt: 0, lainnya: 0 },
    )
  }, [bansosList])
  ...
  const distribution = useMemo(() => {
    ...
    const colors: Record<string, string> = {
      ...
      'Bantuan Dana Pangan': 'bg-amber-500',
      Lainnya: 'bg-gray-400',
    }
  ...
  }, [bansosList, searchQuery, filterJenis])

  // Pagination logic
  const totalPages = Math.ceil(filteredBansos.length / ITEMS_PER_PAGE)
  const paginatedBansos = useMemo(() => {
    const startIndex = (currentPage - 1) * ITEMS_PER_PAGE
    return filteredBansos.slice(startIndex, startIndex + ITEMS_PER_PAGE)
  }, [filteredBansos, currentPage])
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <h3 className="text-3xl font-bold text-gray-800">{kpi.lainnya}</h3>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-8 shadow-sm">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {distribution.map((d, i) => (
            <div
              key={i}
              className="flex flex-col gap-1 p-3 rounded-xl border border-gray-50 hover:bg-gray-50 transition-colors"
            >
              <div className="flex items-center gap-2 mb-1">
                <span className={`w-3 h-3 rounded-full ${d.color}`}></span>
                <span
                  className="text-xs font-semibold text-gray-500 uppercase tracking-wider line-clamp-1"
                  title={d.name}
                >
                  {d.name}
                </span>
              ...
              <div className="flex items-end gap-2">
                ...
                <span className="text-sm text-gray-400 font-medium leading-none mb-0.5">
                  ({d.percentage}%)
                </span>
              </div>
            </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 text-gray-600 text-sm border-b border-gray-100">
                <th className="py-4 px-6 font-semibold w-1/3">Nama & NIK</th>
                <th className="py-4 px-6 font-semibold w-1/3">Alamat</th>
                <th className="py-4 px-6 font-semibold w-1/3">Jenis Bantuan</th>
              </tr>
            </thead>
              ...
              {filteredBansos.length === 0 ? (
                ...
                paginatedBansos.map((item) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-6">
                      <span className="inline-flex items-center gap-1.5 bg-blue-50 text-blue-700 px-3 py-1.5 rounded-full text-xs font-bold whitespace-nowrap">
                        <Gift className="w-3.5 h-3.5" /> {item.jenisBansos}
                      </span>
                    </td>
                  </tr>
                ))
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between mt-6 px-2">
            <p className="text-sm text-gray-500">
              ...
              hingga{' '}
              <span className="font-medium text-gray-900">
                {Math.min(currentPage * ITEMS_PER_PAGE, filteredBansos.length)}
              </span>{' '}
              dari{' '}
              <span className="font-medium text-gray-900">
                {filteredBansos.length}
              </span>{' '}
              data
            </p>
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-2xl p-6 flex items-start gap-4">
        ...
        <div>
          <h4 className="font-bold text-blue-900 mb-1.5">
            Informasi Penyaluran Bansos
          </h4>
          <p className="text-blue-800/80 text-sm leading-relaxed">
            Penyaluran Bantuan Sosial dilakukan secara bertahap sesuai dengan
            jadwal dari Kementerian Sosial dan Pemerintah Daerah. Jika Anda
            merasa berhak namun belum terdaftar, silakan melapor ke Kantor Balai
            Desa dengan membawa fotokopi Kartu Keluarga (KK) dan KTP.
            <strong>
              {' '}
              Transparansi ini disajikan untuk mendukung program keterbukaan
              informasi publik Pemerintah Desa Sambigede.
            </strong>
          </p>
        </div>
      </div>
