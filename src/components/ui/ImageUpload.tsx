...
export function ImageUpload({ value, onChange, onError, className = "" }: ImageUploadProps) {
  ...
  return (
    <div className={`relative w-full overflow-hidden rounded-xl transition-all duration-200 ${className || 'h-48'}`}>
      {value ? (
        ...
        >
          {isUploading ? (
            <div className="flex flex-col items-center text-slate-600">
              ...
              <p className="font-semibold text-xs sm:text-sm">Mengunggah & Mengompresi...</p>
            </div>
          ) : (
            <div className="flex flex-col items-center text-slate-500">
              ...
              <p className="font-semibold text-xs sm:text-sm text-slate-700">
                <span className="text-emerald-600 hover:underline">Klik untuk mengunggah</span>
              </p>
              <p className="text-slate-400 text-xs mt-1">atau seret dan lepas</p>
              <p className="text-[10px] text-slate-400 mt-2 bg-slate-100 px-2 py-0.5 rounded-full">WebP / PNG / JPG</p>
            </div>
          )}
        </div>
      )}
