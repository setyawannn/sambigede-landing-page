...
export const getTurnstileStats = query({
  handler: async (ctx) => {
    ...
    for (const log of recentLogs) {
      if (log.success) {
        ...
        if (log.timestamp > last15Mins) {
          failuresLast15Mins++;
          failedVisitorIds[log.visitorId] = (failedVisitorIds[log.visitorId] || 0) + 1;
        }
      }
    ...
    if (failuresLast15Mins > 15 || (failuresLast15Mins > 5 && successRate < 50)) {
    ...
    return {
      ...
      securityStatus,
      loginPercentage: totalLogs > 0 ? Math.round((loginCount / totalLogs) * 100) : 0,
      pengaduanPercentage: totalLogs > 0 ? Math.round((pengaduanCount / totalLogs) * 100) : 0,
