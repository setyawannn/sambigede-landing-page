...
export const insertPengaduan = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const recentSubmissions = await ctx.db
      .query("pengaduan")
      .withIndex("by_emailOrPhone", (q) => q.eq("emailOrPhone", args.emailOrPhone))
      .filter((q) => q.gt(q.field("_creationTime"), fifteenMinutesAgo))
      .collect();

    if (recentSubmissions.length >= 3) {
      throw new Error("Anda telah mencapai batas maksimal pengiriman. Silakan coba lagi nanti.");
    }
...
export const deleteKategori = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (relatedPengaduan) {
      throw new Error("Kategori tidak dapat dihapus karena masih digunakan pada pengaduan.");
    }
