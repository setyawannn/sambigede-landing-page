...
export const getKelembagaanWithAnggota = query({
  args: {},
  handler: async (ctx) => {
    const lembagaList = await ctx.db.query("kelembagaan").withIndex("by_urutan").collect();
    ...
    const result = await Promise.all(
      lembagaList.map(async (lembaga) => {
        const pengurus = await ctx.db
          ...
          .withIndex("by_kelembagaan", (q) => q.eq("kelembagaanId", lembaga._id))
          .collect();
...
export const getPengurusByKelembagaan = query({
  ...
  handler: async (ctx, args) => {
    const pengurus = await ctx.db
      .query("pengurus_kelembagaan")
      .withIndex("by_kelembagaan", (q) => q.eq("kelembagaanId", args.kelembagaanId))
      .collect();
